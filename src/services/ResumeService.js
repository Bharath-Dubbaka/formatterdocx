        ...
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: `You are a resume section analyzer. Your task is to:
1. Identify each section exactly as written in the resume
2. Keep each section completely separate
3. Do not merge any sections, even if they seem related
4. Preserve original section titles exactly
5. Return a JSON array where each object has:
   - exactTitle: the exact section title as it appears
   - content: the complete content of that section
   - type: the general category (e.g. 'personal', 'experience', 'education', 'projects', 'skills', 'other')
Do not modify or reorganize any content.`
              },
              {
                role: "user",
                content: `Analyze these resume sections, keeping each one separate and preserving original titles:
...
function splitIntoSections(text) {
  ...
  const sectionHeaders = [
    ...
    'CERTIFICATIONS', 'AWARDS', 'PERSONAL DETAILS', 'ADDITIONAL COURSES',
...
function parsePersonalInfo(text) {
  ...
  for (const line of lines) {
    if (line.trim() && !line.includes('@') && !line.includes('Cell:') && !line.includes('Phone:')) {
      info.name = line.trim();
      break;
    }
  }
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.includes('Cell:') || trimmedLine.includes('Phone:')) {
      info.phone = trimmedLine.replace('Cell:', '').replace('Phone:', '').trim();
    }
  }
  ...
  if (summaryIndex !== -1) {
    ...
    info.summary = nextSectionIndex !== -1 
      ? summaryText.slice(0, nextSectionIndex).replace(/summary:?/i, '').trim()
...
function parseWorkExperience(text) {
  const experiences = [];
  const expSection = text.match(/PROFESSIONAL\s+EXPERIENCE\s*:?([\s\S]*?)(?=\n\s*[A-Z][A-Z\s]+:|$)/i);
  ...
  for (const entry of expEntries) {
    ...
    const dateMatch = roleAndDate.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s*\d{4}\s*[-–]\s*(Present|\d{4}|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec\s*\d{4})/i);
    ...
    exp.responsibilities = lines.slice(2)
      .filter(line => line.trim() && !line.toLowerCase().includes('responsibilit'))
...
function parseEducation(text) {
  const education = [];
  const eduSection = text.match(/EDUCATION(?:\s+QUALIFICATION)?:?([\s\S]*?)(?=\n\s*[A-Z][A-Z\s]+:|$)/i);
  ...
  for (const entry of entries) {
    ...
    for (const line of lines) {
      ...
      if (cleanLine.includes('B.Tech') || cleanLine.includes('B.E.') || cleanLine.includes('Bachelor')) {
        edu.degree = 'B.Tech';
      } else if (cleanLine.includes('M.Tech') || cleanLine.includes('M.E.') || cleanLine.includes('Master')) {
        edu.degree = 'M.Tech';
      ...
      // Extract institution
      if (cleanLine.includes('College') || cleanLine.includes('Institute') || cleanLine.includes('School')) {
